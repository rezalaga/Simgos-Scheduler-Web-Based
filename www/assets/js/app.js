function copyResponseBody() {
    ...
    if (!el) return;
    navigator.clipboard.writeText(el.textContent).then(() => toast('Disalin!', 'success')).catch(() => {});
}
...
function refreshDashboard() {
    api('get-stats').then(d => {
        ...
        if (!d.recent_logs.length) {
            cont.innerHTML = '<div class="text-center text-muted py-3">Belum ada eksekusi.</div>';
        } else {
            let html = '<table class="table table-hover mb-0"><thead><tr><th>Waktu</th><th>Tugas</th><th>Server</th><th>Status</th><th>Kode</th><th>Durasi</th></tr></thead><tbody>';
...
function renderActiveTasks() {
    ...
    if (!activeTasks.length) {
        cont.innerHTML = '<div class="text-center text-muted py-4">Tidak ada tugas aktif. <a href="?page=tasks">Buat tugas baru</a>.</div>';
        return;
    }
    const now = Date.now();
    ...
    activeTasks.forEach(t => {
        ...
        html += `<div class="task-donut-card" data-task-id="${t.id}">
            ...
            <div class="task-donut-info">
                ...
                <div class="small text-muted">Berikutnya: ${formatDate(t.next_execution_at)}</div>
            </div>
        </div>`;
    });
...
function executeTask(task) {
    ...
    api('execute-task', { id: task.id }).then(r => {
        if (r.ok) {
            ...
            const label = r.status === 'success' ? 'Sukses' : 'Gagal';
        ...
        delete taskExecuting[task.id];
        toast(`Gagal eksekusi "${task.title}": ${e.message}`, 'error');
        refreshDashboard();
    });
}
...
function loadServers() {
    api('list-servers').then(servers => {
        ...
        if (!servers.length) {
            cont.innerHTML = '<div class="text-center text-muted py-3">Belum ada server. Tambahkan server untuk memulai.</div>';
            return;
        }
        let html = '<table class="table table-hover mb-0"><thead><tr><th style="width:40px">#</th><th>Nama</th><th>URL Dasar</th><th>Dibuat</th><th class="text-end">Aksi</th></tr></thead><tbody>';
        servers.forEach((s, i) => {
            html += `<tr>
                ...
                <td class="text-end">
                    <button class="btn btn-sm btn-outline-primary me-1" onclick="openServerModal(${s.id})" title="Ubah"><i class="bi bi-pencil"></i></button>
                    <button class="btn btn-sm btn-outline-info me-1" onclick="copyServer(${s.id})" title="Salin"><i class="bi bi-copy"></i></button>
                    <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete('server', ${s.id}, '${esc(s.name)}')" title="Hapus"><i class="bi bi-trash"></i></button>
                </td>
            </tr>`;
        });
        html += '</tbody></table>';
        cont.innerHTML = html;
        initTooltips();
    }).catch(e => toast('Gagal memuat server: ' + e.message, 'error'));
}
...
function openServerModal(id) {
    ...
    if (id) {
        document.getElementById('serverModalTitle').textContent = 'Ubah Server';
...
function copyServer(id) {
    api('get-server', { id }).then(s => {
        document.getElementById('serverForm').reset();
        document.getElementById('server-id').value = '';
        document.getElementById('serverModalTitle').textContent = 'Salin Server';
        document.getElementById('server-name').value = s.name + ' (salinan)';
        document.getElementById('server-base-url').value = s.base_url;
        new bootstrap.Modal(document.getElementById('serverModal')).show();
    }).catch(e => toast('Gagal menyalin server: ' + e.message, 'error'));
}
...
function saveServer(e) {
    ...
    }).catch(e => toast('Gagal menyimpan server: ' + e.message, 'error'));
    return false;
}
...
function loadTasks() {
    api('list-tasks').then(tasks => {
        ...
        if (!tasks.length) {
            cont.innerHTML = '<div class="text-center text-muted py-3">Belum ada tugas. Tambahkan tugas untuk memulai penjadwalan.</div>';
            return;
        }
        let html = '<table class="table table-hover mb-0"><thead><tr><th style="width:40px">#</th><th style="width:40px"><input type="checkbox" onchange="toggleAllTaskCheckboxes(this)"></th><th>Judul</th><th>Server</th><th>Path</th><th>Status</th><th>Interval</th><th>Jadwal</th><th class="text-end">Aksi</th></tr></thead><tbody>';
        tasks.forEach((t, i) => {
            ...
            html += `<tr class="${active ? '' : 'opacity-50'}">
                ...
                <td>${active ? '<span class="badge bg-success">Aktif</span>' : '<span class="badge bg-secondary">Nonaktif</span>'}</td>
                <td>${t.execute_interval_sec}s</td>
                <td><small class="text-muted">${formatDate(t.next_execution_at)}</small></td>
                <td class="text-end">
                    <button class="btn btn-sm btn-outline-success me-1" onclick="runTask(${t.id}, '${esc(t.title)}')" title="Jalankan Sekarang"><i class="bi bi-play-fill"></i></button>
                    <button class="btn btn-sm btn-outline-${active ? 'warning' : 'success'} me-1" onclick="toggleTask(${t.id})" title="${active ? 'Nonaktifkan' : 'Aktifkan'}"><i class="bi bi-${active ? 'pause' : 'play'}"></i></button>
                    <button class="btn btn-sm btn-outline-primary me-1" onclick="openTaskModal(${t.id})" title="Ubah"><i class="bi bi-pencil"></i></button>
                    <button class="btn btn-sm btn-outline-info me-1" onclick="copyTask(${t.id})" title="Salin"><i class="bi bi-copy"></i></button>
                    <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete('task', ${t.id}, '${esc(t.title)}')" title="Hapus"><i class="bi bi-trash"></i></button>
                </td>
            </tr>`;
        });
        html += '</tbody></table>';
...
function openTaskModal(id) {
    ...
    api('list-servers').then(servers => {
        ...
        if (id) {
            document.getElementById('taskModalTitle').textContent = 'Ubah Tugas';
...
function copyTask(id) {
    api('get-task', { id }).then(t => {
        api('list-servers').then(servers => {
            ...
            document.getElementById('taskForm').reset();
            document.getElementById('task-id').value = '';
            document.getElementById('taskModalTitle').textContent = 'Salin Tugas';
            document.getElementById('task-title').value = t.title + ' (salinan)';
            document.getElementById('task-path').value = t.path;
            document.getElementById('task-interval').value = t.execute_interval_sec;
            document.getElementById('task-error-interval').value = t.error_interval_sec;
            document.getElementById('task-timeout').value = t.response_timeout_ms;
    ...
    }).catch(e => toast('Gagal menyalin tugas: ' + e.message, 'error'));
}
...
function saveTask(e) {
    ...
    }).catch(e => toast('Gagal menyimpan tugas: ' + e.message, 'error'));
    return false;
}
...
function runTask(id, title) {
    api('execute-task', { id }).then(r => {
        ...
        loadTasks();
    }).catch(e => toast(`Gagal mengeksekusi "${title}": ${e.message}`, 'error'));
}
...
function bulkTaskAction(action, label) {
    ...
    if (!ids) { toast('Tidak ada tugas dipilih', 'warning'); return; }
    api(action, { ids }).then(d => {
        if (d.ok) { toast(label + ' ' + ids.split(',').length + ' tugas', 'success'); loadTasks(); }
    }).catch(e => toast('Gagal: ' + e.message, 'error'));
}

function startSelectedTasks() { bulkTaskAction('start-selected-tasks', 'Dimulai'); }
function stopSelectedTasks() { bulkTaskAction('stop-selected-tasks', 'Dihentikan'); }

function startAllTasks() {
    api('start-all-tasks').then(d => {
        if (d.ok) { toast('Semua tugas dimulai', 'success'); loadTasks(); }
    }).catch(e => toast('Gagal: ' + e.message, 'error'));
}

function stopAllTasks() {
    if (!confirm('Nonaktifkan semua tugas?')) return;
    api('stop-all-tasks').then(d => {
        if (d.ok) { toast('Semua tugas dihentikan', 'success'); loadTasks(); }
    }).catch(e => toast('Gagal: ' + e.message, 'error'));
}
...
function toggleTask(id) {
    api('toggle-task', { id }).then(d => {
        if (d.ok) {
            toast('Tugas diubah', 'success');
            loadTasks();
        }
    }).catch(e => toast('Gagal mengubah tugas: ' + e.message, 'error'));
}
...
function loadLogs(page) {
    ...
    api('list-logs', { page: p, status, task_id: taskFilter }).then(d => {
        ...
        if (!d.logs.length) {
            cont.innerHTML = '<div class="text-center text-muted py-3">Belum ada log.</div>';
            return;
        }
        let html = '<table class="table table-hover mb-0"><thead><tr><th>Waktu</th><th>Tugas</th><th>URL</th><th>Status</th><th>Kode</th><th>Durasi</th><th class="text-end">Aksi</th></tr></thead><tbody>';
        d.logs.forEach(l => {
...
function viewLog(id) {
    ...
    document.getElementById('log-detail-body').innerHTML = '<div class="text-center text-muted py-3">Memuat...</div>';
    modal.show();
    api('get-log', { id }).then(l => {
        if (!l) {
            document.getElementById('log-detail-body').innerHTML = '<div class="text-center text-muted py-3">Log tidak ditemukan</div>';
            return;
        }
        const body = l.response_body_pretty
            ? `<pre class="mb-0" id="log-response-body">${esc(l.response_body_pretty)}</pre>`
            : '<div class="text-muted">Tidak ada respons</div>';
        let respSection;
        if (l.response_body) {
            respSection = `<div class="d-flex align-items-center gap-2 mt-3 mb-2">
                <h6 class="text-muted mb-0">Respons (Cetak Rapi)</h6>
                <button class="btn btn-sm btn-outline-secondary py-0" onclick="copyResponseBody()" title="Salin respons"><i class="bi bi-clipboard"></i></button>
            </div>${body}
                <details class="mt-2"><summary class="text-muted small cursor-pointer">Lihat mentah</summary>
                <pre class="mt-2">${esc(l.response_body)}</pre></details>`;
        } else {
            respSection = '<div class="text-muted mt-3">Tidak ada respons</div>';
        }

        document.getElementById('log-detail-body').innerHTML = `
            <div class="row mb-3">
                <div class="col-md-4"><small class="text-muted d-block">Tugas</small><strong>${esc(l.task_title || l.title)}</strong></div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4"><small class="text-muted d-block">Kode HTTP</small><strong>${l.http_code || '-'}</strong></div>
                <div class="col-md-4"><small class="text-muted d-block">Waktu Respon</small><strong>${l.response_time_ms || '-'} ms</strong></div>
            </div>
            ${l.error_message ? `<div class="alert alert-danger py-2"><i class="bi bi-exclamation-triangle me-1"></i>${esc(l.error_message)}</div>` : ''}
            ${respSection}
            <small class="text-muted d-block mt-3">Dieksekusi pada: ${formatDate(l.executed_at)}</small>
        `;
    }).catch(e => {
        const info = { name: e.name, message: e.message, stack: e.stack };
        document.getElementById('log-detail-body').innerHTML = `<div class="alert alert-danger">Gagal memuat log: ${esc(info.name)}: ${esc(info.message)}</div>`;
        console.error('viewLog error:', info, 'id:', id);
    });
}

function clearLogs() {
    if (!confirm('Hapus semua log eksekusi? Tindakan ini tidak bisa dibatalkan.')) return;
    api('clear-logs').then(d => {
        if (d.ok) {
            toast('Semua log dihapus', 'success');
            refreshLogs();
        }
    }).catch(e => toast('Gagal menghapus log: ' + e.message, 'error'));
}
...
function confirmDelete(type, id, name) {
    ...
    document.getElementById('deleteModalBody').innerHTML = `Hapus <strong>${esc(name)}</strong>? Log terkait juga akan dihapus.`;
    document.getElementById('deleteConfirmBtn').onclick = function() {
        api(`delete-${deleteTarget.type}`, { id: deleteTarget.id }).then(d => {
            if (d.ok) {
                bootstrap.Modal.getInstance(document.getElementById('deleteModal')).hide();
                const label = deleteTarget.type === 'server' ? 'Server dihapus' : 'Tugas dihapus';
                toast(label, 'success');
        ...
        cont.innerHTML = `
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="form-text">Zona waktu server untuk penjadwalan dan tampilan jam.</div>
                    <button class="btn btn-primary mt-3" onclick="saveSettings()"><i class="bi bi-check-lg"></i> Simpan</button>
                </div>
            </div>`;
    }).catch(e => {
        const cont = document.getElementById('settings-container');
        if (cont) cont.innerHTML = `<div class="alert alert-danger">Gagal memuat pengaturan: ${e.message}</div>`;
    });
}
...
function saveSettings() {
    ...
    api('save-settings', { timezone: tz }, 'POST').then(r => {
        if (r.ok) {
            document.getElementById('sidebar-tz-iana').textContent = tz;
            toast('Zona waktu disimpan ke ' + tz, 'success');
        }
